"""本地加密文件存储 (EAIF1/EAIF2 兼容格式)。

格式约定：
    EAIF1: 5 字节 magic + 12 字节 nonce + AES-GCM(ciphertext + 16B tag)
    EAIF2: 5 字节 magic + 1 字节 version_length + N 字节 version + 12 字节 nonce
            + AES-GCM(ciphertext + 16B tag, AAD 含 storage_key 与 version)
"""
